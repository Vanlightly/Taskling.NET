...
namespace Taskling.SqlServer.IntegrationTest.Given_BlockService
{
    ...
    public class When_FindFailedRangeBlocks
    {
        ...
        {
            // ARRANGE
            var taskExecution1 = _executionHelper.InsertTaskExecution(_taskSecondaryId);
            var block1 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-2), DateTime.UtcNow.AddMinutes(-1));
            var block2 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-12), DateTime.UtcNow.AddMinutes(-11));
        ...
        public void When_FailedBlocksExistInTargetPeriodAndNumberIsGreaterThanBlocksLimit_ThenReturnOldestBlocksUpToCountLimit()
        {
            ...
            var block1 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-2), DateTime.UtcNow.AddMinutes(-1));
            var block2 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-12), DateTime.UtcNow.AddMinutes(-11));
            var block3 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-22), DateTime.UtcNow.AddMinutes(-21));
            var block4 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-32), DateTime.UtcNow.AddMinutes(-31));
            _blocksHelper.InsertDateRangeBlockExecution(taskExecution1, block1, DateTime.UtcNow.AddMinutes(-2), DateTime.UtcNow.AddMinutes(-1), BlockExecutionStatus.Failed);
            _blocksHelper.InsertDateRangeBlockExecution(taskExecution1, block2, DateTime.UtcNow.AddMinutes(-12), DateTime.UtcNow.AddMinutes(-11), BlockExecutionStatus.Failed);
            _blocksHelper.InsertDateRangeBlockExecution(taskExecution1, block3, DateTime.UtcNow.AddMinutes(-22), DateTime.UtcNow.AddMinutes(-21), BlockExecutionStatus.Failed);
            _blocksHelper.InsertDateRangeBlockExecution(taskExecution1, block4, DateTime.UtcNow.AddMinutes(-32), DateTime.UtcNow.AddMinutes(-31), BlockExecutionStatus.Completed);
        ...
        public void When_FailedBlocksExistOutsideTargetPeriod_ThenReturnNoBlocks()
        {
            // ARRANGE
            var taskExecution1 = _executionHelper.InsertTaskExecution(_taskSecondaryId);
            var block1 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-200), DateTime.UtcNow.AddMinutes(-201));
            var block2 = _blocksHelper.InsertDateRangeBlock(_taskSecondaryId, DateTime.UtcNow.AddMinutes(-212), DateTime.UtcNow.AddMinutes(-211));
