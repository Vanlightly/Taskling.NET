...
namespace Taskling.SqlServer.IntegrationTest.Repositories.Given_BlockRepository
{
    ...
    public class When_FindDeadBlocks
    {
        ...
        public void When_OverrideModeAndList_DeadTasksInTargetPeriodAndLessThanBlockCountLimit_ThenReturnAllDeadBlocks()
        {
            ...
            // ASSERT
            Assert.AreEqual(3, deadBlocks.Count);
            Assert.IsTrue(deadBlocks.Any(x => x.ListBlockId == _block2));
            Assert.IsTrue(deadBlocks.Any(x => x.ListBlockId == _block3));
            Assert.IsTrue(deadBlocks.Any(x => x.ListBlockId == _block5));
        }
        ...
        public void When_OverrideModeAndObject_DeadTasksInTargetPeriodAndLessThanBlockCountLimit_ThenReturnAllDeadBlocks()
        {
            ...
            // ASSERT
            Assert.AreEqual(3, deadBlocks.Count);
            Assert.IsTrue(deadBlocks.Any(x => x.ObjectBlockId == _block2));
            Assert.IsTrue(deadBlocks.Any(x => x.ObjectBlockId == _block3));
            Assert.IsTrue(deadBlocks.Any(x => x.ObjectBlockId == _block5));
        }
